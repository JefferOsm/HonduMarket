...
function EditarArticulo() {
  ...
  const { obtenerImagenes, obtenerDetalles, detailProduct, imagenesProduct,
    videoProduct, obtenerUsuario, eliminarImagenProd, eliminarVideo } = usarProductosContex();
  ...
  // estado para almacenar los videos a eliminar
  ...
  const eliminarVideoCaruosel = () => {
    // Establecer videoPreview a una cadena vacía para eliminar el video
    setVideoPreview(prevState => prevState === '' ? ' ' : '');

    // Si videoProduct tiene un id_video, agregarlo a un estado de videos para eliminar
    if (videoProduct[0] && videoProduct[0].id_video) {
      setVideosEliminar(prevState => [...prevState, videoProduct[0].id_video]);
    }
  ...
  let vistaPreviaVideo;
  if (videoPreview) {
    vistaPreviaVideo = (
      <div className="carousel-item text-center">
        <ReactPlayer className='d-block w-100' url={videoPreview} controls />
        <button type="button" className="btn-delete p-2 rounded mx-auto"
          onClick={eliminarVideoCaruosel}><FontAwesomeIcon icon={faTrash} /></button>
      </div>
    );
  } else if (videoProduct.length > 0) {
    vistaPreviaVideo = (
      <div className="carousel-item text-center">
        <ReactPlayer className='d-block w-100' url={videoProduct[0].url_video} controls />
        <button type="button" className="btn-delete p-2 rounded mx-auto"
          onClick={eliminarVideoCaruosel}><FontAwesomeIcon icon={faTrash} /></button>
      </div>
    );
            ...
            <div className="d-flex gap-5">
              ...
              <div className="col-md-5">
                ...
                <input
                  ...
                  {...register('imagenes', {
                    onChange: (e) => { generarVistaPreviaImagenes(e) }
